...
var jqCronDefaultSettings = {
	texts: {
		en : {
			...
			text_mins: _('at')+' <b /> '+ _('minutes_past_the_hour'),
			text_time: _('at')+' <b />:<b />',
			text_dow: _('on')+' <b />',
			text_month: _('of')+' <b />',
			text_dom: _('on_the')+' <b />',
			error1: _('the_tag_%s_is_not_supported'),
			error2: _('bad_number_of_elements'),
...
(function($)
{
	$.fn.jqCron = function(settings) {
		return this.each(function() {
			...
			settings = jqCronMergeSettings($this.data()); // clone settings from data-attributes
